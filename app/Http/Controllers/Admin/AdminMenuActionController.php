    ...
    public function index($id)
    {
        if (request()->ajax()) {
            ...
            ->addColumn('status', function($row){
                ...
                if($row->status == 1){
                    $status .= '<input class="form-check-input c-pointer change-status" type="checkbox" data-url="'.route('admin.admin-menuAction.status',$row->id).'" id="'.$row->id.'" checked>
                                <label class="form-check-label" for="'.$row->id.'"></label>';
                } else {
                    $status .= '<input class="form-check-input c-pointer change-status" type="checkbox" data-url="'.route('admin.admin-menuAction.status',$row->id).'" id="'.$row->id.'">
                    <label class="form-check-label" for="'.$row->id.'"></label>';
                }
                $status .= '</div>';
                return $status;
